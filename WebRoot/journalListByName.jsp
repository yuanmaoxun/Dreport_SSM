...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-table"></i> 工作量统计 <span>按人员进行工作量统计</span></h2>
      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          ...
          <table class="table table-striped" id="table2">
              ...
              <tbody>
                 <c:forEach items="${journalinfo}" var="item">
  				<tr>
	  				<td>${item.empNum}</td>
	  				<td class="x">${item.empUsername}</td>
	  				<td>${item.proName}</td>	
	  				<td class="y1">${item.jourWorkSumTime}</td>
	  				<td class="y2">${item.jourOverWorkSumTime}</td>
	  				<td><fmt:formatNumber value="${item.jourOverWorkSumTime*100/(item.jourWorkSumTime+item.jourOverWorkSumTime)}" maxFractionDigits="2" minFractionDigits="2" ></fmt:formatNumber>%</td>
	  			</tr>
  			</c:forEach>
              </tbody>
           </table>
...
<script src="/dreport/js/snovey-lib.js"></script>
...
<script>
  jQuery(document).ready(function() {
    ...
    "use strict";
    var chartsdata = $('tbody tr').getTableData(['x', 'y1', 'y2']);
    Morris.Area({
      element: 'charts',
      data: chartsdata,
      xkey: 'x',
      ykeys: ['y1', 'y2'],
      labels: ['工作总代码量', '加班总代码量'],
      parseTime: false
    });
