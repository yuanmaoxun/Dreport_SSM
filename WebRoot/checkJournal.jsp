...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-table"></i> 审核日报 <span>请在此审核需要您进行审核的日报</span></h2>
      <div class="breadcrumb-wrapper">
        <span class="label">您目前在:</span>
        <ol class="breadcrumb">
          <li><a href="/dreport/index.jsp">Dreport</a></li>
          <li class="active">审核日报</li>
        </ol>
      </div>
    </div>
              ...
              <tbody>
                 <c:forEach items="${journalList}" var="item">
  					<tr>
	  					...
	  					<td  id="proPrpStage" height="20" align="center" name="proPrp">${item.proPrpStage}</td>
	  					<td>${item.jourWorkload}</td>
	  					<td>${item.proName}</td>
	  					<td> <fmt:formatDate value="${item.jourSubmitTime}" /></td>
              ...
              <c:choose>
                ...
                <c:otherwise>
                  <td>审核状态出错</td>
	  					...
	  					<td><a href="${pageContext.request.contextPath }/journalController/journalAllowed.action?jourid=${item.jourId}"><i class="glyphicon glyphicon-saved"></i></a>
	  												<a href="${pageContext.request.contextPath }/journalController/journalUnallowed.action?jourid=${item.jourId}" ><i class="glyphicon glyphicon-remove-circle"></i></a>
	  					</td>
	  				</tr>
				</c:forEach>
              </tbody>
